...
fn error() -> CSDeliverError {
    CSDeliverError::DeserializationError("Input bytes too small or not valid".to_owned())
}

/// Extract u32 from the source [cursor]
pub(crate) fn read_u32(cursor: &mut Cursor<&[u8]>) -> Result<u32, CSDeliverError> {
...
pub fn read_vector(cursor: &mut Cursor<&[u8]>) -> Result<Vec<u8>, CSDeliverError> {
