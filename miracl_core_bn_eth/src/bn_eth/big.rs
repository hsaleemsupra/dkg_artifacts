...
// Hamza
    // Implements method to compute inverse using mod exp method
    // Uses the algorithm specified in ModexpSqrt: https://github.com/thehubbleproject/hubble-contracts/blob/master/contracts/libs/ModExp.sol#L337
    //inv(input) = input^{(N + 1) / 4} mod N
    pub fn modexpsqrt(&mut self) -> BIG {
        let n = BIG::new_ints(&crate::bn_eth::rom::MODULUS);
        let t6 = BIG::new_big(&self);
        let mut t0 = BIG::modmul(&t6, &t6, &n);
        let t4 = BIG::modmul(&t0, &t6, &n);
        let t2 = BIG::modmul(&t4, &t0, &n);
        let mut t3 = BIG::modmul(&t4, &t4, &n);
        let t8 = BIG::modmul(&t2, &t0, &n);
        let t1 = BIG::modmul(&t3, &t4, &n);
        let t5 = BIG::modmul(&t3, &t2, &n);
        t0 = BIG::modmul(&t3, &t3, &n);
        let t7 = BIG::modmul(&t8, &t3, &n);
        t3 = BIG::modmul(&t1, &t3, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t4, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t6, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
        t0 = BIG::modmul(&t0, &t0, &n);
