...
use crate::bn_eth::big;
use crate::bn_eth::big::BIG;
use crate::bn_eth::dbig::DBIG;
use crate::bn_eth::fp::FP;
use crate::bn_eth::fp;
use crate::bn_eth::rom;
...
// Set P=e*P
    ...
    //Hamza: implements the algorithm from the method mapToPoint in https://github.com/thehubbleproject/hubble-contracts/blob/master/contracts/libs/BLS.sol#L165
    /* Constant time Map to Point */
    #[allow(non_snake_case)]
    pub fn map2point(h: &FP) -> ECP {
        let mut P = ECP::new();

        let x_sqrt = FP::new_big(&h.redc().modexpsqrt());
        let mut xx = FP::new_copy(&x_sqrt);
        xx.mul(&FP::new_copy(&xx));
        let mut decision = false;
        if xx.equals(h) {
            decision = true;
        }
        let mut a0 = FP::new_copy(&h);
        a0.mul(&a0.clone());
        a0.add(&FP::new_int(4));
        a0.reduce();
        // sqrt(-3)
        let Z0 = FP::new_big(&BIG::new_ints(&rom::Z0));
        // (sqrt(-3) - 1)  / 2
        let Z1 = FP::new_big(&BIG::new_ints(&rom::Z1));
        let mut a1 = FP::new_copy(&h);
        a1.mul(&Z0);
        let mut a2 = FP::new_copy(&a1);
        a2.mul(&a0.clone());
        a2.inverse(None);
        a1.mul(&a1.clone());
        a1.mul(&a2);
        // x1
        a1.mul(&FP::new_copy(&h));
        let mut x = FP::new_copy(&Z1);
        let mut N_min_a1 = FP::new_copy(&a1);
        N_min_a1.neg();
        x.add(&N_min_a1);
        x.reduce();
        // check curve
        ...
        a1.reduce();
        let mut found = false;
        let a1_sqrt = FP::new_big(&a1.redc().modexpsqrt());
        let mut a1a1 = FP::new_copy(&a1_sqrt);
        a1a1.mul(&a1a1.clone());
        if a1a1.equals(&a1) {
            found = true;
        }
    ...
    // David: based on Hamza's implementation in the maptopoint-fix branch.
    pub fn map2point_ft(h: &FP) -> Self {
        let decision = h.qr(None) == 1;

        let mut a0 = FP::new_copy(&h);
        a0.sqr();
        a0.add(&FP::new_int(4));

        let Z0 = FP::new_big(&BIG::new_ints(&rom::SQRTM3));

        let Z1 = {
            let mut sqrtm3 = FP::new_copy(&Z0);
            let one = FP::new_int(1);
            sqrtm3.sub(&one);
            sqrtm3.div2();
            sqrtm3
        ...
        let mut hint = FP::new();
        a1 = ECP::rhs(&x);
        if a1.qr(Some(&mut hint)) == 1 {
            let mut y = a1.sqrt(Some(&hint));
            if !decision {
                y.neg();
            }
            return ECP::new_bigs(&x.redc(), &y.redc());
        }
        ...
        a1 = ECP::rhs(&x);
        let mut y = a1.sqrt(None);
        if !decision {
            y.neg();
        }
        ECP::new_bigs(&x.redc(), &y.redc())
    }
...
#[cfg(test)]
mod test {
    use miracl_core_bls12381::rand::{RAND, RAND_impl};
    use crate::bn_eth::big::MODBYTES;
    use crate::bn_eth::pair;
    use rand::prelude::*;
    use super::*;
    ...
    fn test_mul_add_dbl() {
        ...
        let mut pp = g.clone();
        pp.add(&g);
        assert!(pp.equals(&g.mul(&BIG::new_int(2))));
        let mut qq = g.clone();
        qq.dbl();
        assert!(qq.equals(&g.mul(&BIG::new_int(2))));
    }
    ...
    // David: slow, compile with "--release"
    #[test]
    fn test_map2point() {
        for _ in 0..5000 {
            ...
            let mut rng = RAND_impl::new();
            rng.seed(seed.len(), seed.as_slice());

            let h = FP::new_rand(&mut rng);
            let point = ECP::map2point(&h);
            let x = point.getpx();
            let rhs = ECP::rhs(&x);
            let mut y_sqr = point.getpy();
            y_sqr.sqr();
            // println!("{}", x);

            assert!(y_sqr.equals(&rhs));
    ...
    // David: (-1)^3 + 3 = 2 happens to be a whole square in FP. This explains why Z = -1 works for ECP.
    // in Map2Point. Z = 1 obviously works since 1^3 + 3 = 4 = sqr(2).
    #[test]
    fn test_sqrt_rhs_z_m1() {
        let z = FP::new_int(-1);
        let rhs = ECP::rhs(&z);
        assert_eq!(rhs.qr(None), 0);
    }
