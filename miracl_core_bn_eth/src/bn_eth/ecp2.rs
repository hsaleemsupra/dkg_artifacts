...
use crate::bn_eth::ecp;
use crate::bn_eth::fp2::FP2;
...
impl ECP2 {
    ...
    pub fn map2point_ft(h: &FP2) -> Self {
        let mut h_copy = FP2::new_copy(h);
        let decision = h_copy.qr(None) == 1;

        let mut a0 = FP2::new_copy(&h);
        a0.sqr();
        {
            let mut temp = FP2::new_int(rom::CURVE_B_I);
            temp.div_ip();
            temp.add(&FP2::new_int(1));
            a0.add(&temp);
        }

        let Z0 = FP2::new_big(&BIG::new_ints(&rom::SQRTM3));

        let Z1 = {
            let mut sqrtm3 = FP2::new_copy(&Z0);
            let one = FP2::new_int(1);
            sqrtm3.sub(&one);
            sqrtm3.div2();
            sqrtm3
        ...
        let mut a1 = FP2::new_copy(&h);
        a1.mul(&Z0);
        let mut a2 = FP2::new_copy(&a1);
        a2.mul(&a0);
        a2.inverse(None);
        a1.sqr();
        a1.mul(&a2);
        ...
        // x1
        a1.mul(&FP2::new_copy(&h));
        let mut x = FP2::new_copy(&Z1);
        let mut N_min_a1 = FP2::new_copy(&a1);
        N_min_a1.neg();
        x.add(&N_min_a1);
...
mod test {
    ...
    fn test_map2point() {
        for _ in 0..5000 {
            ...
            let h = FP2::new_rand(&mut rng);
            let point = ECP2::map2point(&h);
            let x = point.getpx();
            let rhs = ECP2::rhs(&x);
    ...
    // David: (-1)^3 + 3/(9+i) is NOT a whole square. This explains why Z = -1 doesn't work for ECP2.
    // in Map2Point.
    #[test]
    fn test_sqrt_rhs_z_m1() {
        let z = FP2::new_int(-1);
        let mut rhs = ECP2::rhs(&z);
        assert_eq!(rhs.qr(None), 0);
    }

    // David: 1^3 + 3/(9+i) is a whole square. This explains why Z = 1 works for ECP2 in Map2Point.
    #[test]
    fn test_sqrt_rhs_z_1() {
        let z = FP2::new_int(1);
        let mut rhs = ECP2::rhs(&z);
        assert_eq!(rhs.qr(None), 1);
    }
}