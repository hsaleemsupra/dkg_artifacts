...
use crate::types::impls::threshold_bls::mcl::BlsThresholdMcl;
use crate::types::Order;
pub(crate) const DOMAIN_BLS_THRESHOLD_MCL: &str = "bls-threshold-signature-mcl";

/// Threshold signature domain definition based on BLS12381 curve.
pub struct BlsThresholdDomainMcl;
impl UniqueDomain for BlsThresholdDomainMcl {
    fn domain<'a>() -> &'a str {
        DOMAIN_BLS_THRESHOLD_MCL
    }
}

/// Bls Threshold aggregated signature schema based on BLS12381 curve.
pub type BlsThresholdMclSchema = BlsThresholdMcl<BlsThresholdDomainMcl>;

/// Aggregate Signature Wrapper struct for bls threshold signature on BLS12381 curve.
pub type AggregatedSignatureBlsThresholdMcl = AggregatedSignatureWrapper<BlsThresholdMclSchema>;
/// Partial Signature Wrapper struct for bls threshold signature on BLS12381 curve.
pub type PartialSignatureBlsThresholdMcl = PartialSignatureWrapper<BlsThresholdMclSchema>;
/// Public Key Wrapper struct for bls threshold signature on BLS12381 curve.
pub type PublicKeyBlsThresholdMcl = PublicKeyWrapperAggSig<BlsThresholdMclSchema>;
/// Public Parameters Wrapper struct for bls threshold signature on BLS12381.
pub type PublicParametersBlsThresholdMcl = PublicParametersWrapperAggSig<BlsThresholdMclSchema>;
/// Signing Key Wrapper struct for bls threshold signature on BLS12381.
pub type SigningKeyBlsThresholdMcl = SigningKeyWrapperAggSig<BlsThresholdMclSchema>;
/// Verification Key Wrapper struct for bls threshold signature on BLS12381.
pub type VerificationKeyBlsThresholdMcl = VerificationKeyWrapperAggSig<BlsThresholdMclSchema>;
...
pub fn trusted_dkg_bls_threshold_mcl(
    thresh: Order,
    num_nodes: Order,
) -> (
    Vec<SigningKeyBlsThresholdMcl>,
    PublicParametersBlsThresholdMcl,
) {
...
mod test {
    ...
    #[test]
    fn check_threshold_mcl_signing_api_wrappers_trusted_dkg() {
        let (sks, pp) = super::trusted_dkg_bls_threshold_mcl(Order(THRESHOLD), Order(NUM_NODES));
