...
use crypto::bls12381::bls_signature::verify_point_ecp2;
use crypto::bls12381::hash_to_point::hash_to_ecp2;
use crypto::bls12381::interpolate::interpolate_g2;
use crypto::bls12381::nizk_dleq::{prove_gen_2, DLEqInstance_2, DLEqWitness};
...
pub(crate) type VerificationKey = Ecp2Wrapper;
pub(crate) type AggregatedSignature = ECP2;
pub(crate) type PublicKey = ECP;
...
pub struct RevBlsThresholdBls12381<T: UniqueDomain>(PhantomData<T>);

impl<DST: UniqueDomain> AggregateSignatureScheme for RevBlsThresholdBls12381<DST> {
    ...
    fn sign<T: AsRef<[u8]>>(
        ...
        let h = hash_to_ecp2(msg.as_ref(), DST::domain().as_bytes());

        let h_x = g2mul(&h, &sk.0);
        let signature = Ecp2Wrapper(h_x.clone());

        let inst = DLEqInstance_2 {
            g: ECP2::generator(),
            h,
        ...
        let proof = prove_gen_2(&inst, &wit);
    ...
    >(
        ...
        let mut evals: Vec<(BIG, ECP2)> = psigs
            .into_iter()
        ...
        // interpolate_g2 function requires a set of THRESHOLD evaluations
        evals.truncate(pp.threshold());
        Ok(interpolate_g2(&evals).expect("Internal: interpolate_g2 should not throw errors"))
    }
    ...
    fn verify_aggregated_signature<T: AsRef<[u8]>>(
        ...
        let msg_hash = hash_to_ecp2(msg.as_ref(), DST::domain().as_ref());
        verify_point_ecp2(&msg_hash, sig, pk).then_some(()).ok_or(
    ...
    fn sign_no_vk<T: AsRef<[u8]>>(
        ...
        let vk = Ecp2Wrapper(g2mul(&ECP2::generator(), &sk.0 .0));
        Self::sign(msg, sk, &vk)
    }
}

// EVENTUALLY GENERIC IMPLEMENTATION BASED ON DKG SCHEME PREFERABLE
impl<DST: UniqueDomain> ThresholdSignatureScheme for RevBlsThresholdBls12381<DST> {
    ...
    fn trusted_dkg(
        ...
        for i in 0..num_parties.index() {
            ...
            let vk_i = g2mul(&gen, &sk_i);

            sks.push(SecretWrapper(sk_i.into()));
            vk_map.insert(id, Ecp2Wrapper(vk_i));
        }

        let committee_sk_big = poly.evaluate_at(&BIG::new_int(0));
        let pk = g1mul(&ECP::generator(), &committee_sk_big);
...
mod tests {
    ...
    use crate::api::instances::rev_bls_threshold_bls12381::{
        ReverseBlsThresholdBls12381Schema, DOMAIN_REV_BLS_THRESHOLD_BLS12381,
    };
    use crate::types::impls::helpers::bls12381::big_as_sk::BigSk;
    use crate::types::impls::helpers::bls12381::ecp2_wrapper::Ecp2Wrapper;
    use crate::types::impls::helpers::rand::rng_from_seed;
    ...
    fn vk_from_sk(sk: &SecretWrapper<BigSk>) -> Ecp2Wrapper {
        Ecp2Wrapper(g2mul(&ECP2::generator(), &sk.0))
    }

    // Note this also checks partial_signature::PartialSignature, later we can separate out the tests
    #[test]
    fn check_rev_threshold_bls_12381_signing_api_partial_signature() {
        let signing_key = new_sk();
        let expected_vk = vk_from_sk(&signing_key);
        let msg = b"test_message";
        let other_msg = b"test_other_message";
        let expected_signature = g2mul(
            &hash_to_ecp2(msg, DOMAIN_REV_BLS_THRESHOLD_BLS12381.as_bytes()),
            &signing_key.0,
        );

        let psig_with_vk = ReverseBlsThresholdBls12381Schema::sign(msg, &signing_key, &expected_vk);
        let psig_no_vk = ReverseBlsThresholdBls12381Schema::sign_no_vk(msg, &signing_key);
        let psig_other_message =
            ReverseBlsThresholdBls12381Schema::sign(other_msg, &signing_key, &expected_vk);
        ...
        assert!(psig_with_vk
            .validate(msg, &expected_vk, DOMAIN_REV_BLS_THRESHOLD_BLS12381)
            .is_ok());
        assert!(psig_no_vk
            .validate(msg, &expected_vk, DOMAIN_REV_BLS_THRESHOLD_BLS12381)
            .is_ok());
        assert!(psig_other_message
            .validate(other_msg, &expected_vk, DOMAIN_REV_BLS_THRESHOLD_BLS12381)
            .is_ok());
        ...
        let result = ReverseBlsThresholdBls12381Schema::verify_partial_signature(
            msg,
            &psig_with_vk,
            &expected_vk,
        );
        ...
        let result =
            ReverseBlsThresholdBls12381Schema::verify_partial_signature(msg, &psig_no_vk, &bad_vk);
        assert!(result.is_err(), "{:?}", result);
    ...
    #[test]
    fn check_rev_threshold_bls_12381_signing_api_trusted_dkg() {
        let (sks, pp) =
            ReverseBlsThresholdBls12381Schema::trusted_dkg(Order(THRESHOLD), Order(NUM_NODES));
        ...
        for (i, sk_i) in sks.iter().enumerate().take(NUM_NODES as usize) {
            ...
            let psig = ReverseBlsThresholdBls12381Schema::sign(&msg, sk_i, vk_i);

            assert!(
                ReverseBlsThresholdBls12381Schema::verify_partial_signature(&msg, &psig, vk_i)
                    .is_ok()
            );
            assert!(ReverseBlsThresholdBls12381Schema::verify_partial_signature(
                &other_msg, &psig, vk_i
            )
            .is_err());
        ...
        let aggregated_signature =
            ReverseBlsThresholdBls12381Schema::aggregate_partial_signatures(&msg, psigs, &pp)
                .expect("should not panic on >=THREHSOLD honest inputs");

        assert!(
            ReverseBlsThresholdBls12381Schema::verify_aggregated_signature(
                &msg,
