...
pub struct CLBox(pub cpp_core::CppBox<CLHSMqk>);
...
unsafe impl Send for CLBox {}
unsafe impl Sync for MpzBox {}
unsafe impl Sync for QFIBox {}
unsafe impl Sync for CiphertextBox {}
unsafe impl Sync for SecretKeyBox {}
unsafe impl Sync for PublicKeyBox {}
unsafe impl Sync for CLBox {}
...
impl CiphertextBox{
    ...
    pub unsafe fn add(&self, c: &cpp_core::CppBox<CLHSMqk>, other: &CiphertextBox) -> CiphertextBox {

        let ref_c: cpp_core::Ref<CLHSMqk> = cpp_core::Ref::from_raw_ref(c);
        let ref_cipher_1: cpp_core::Ref<CLHSMCipherTextOfCLHSMqk> = cpp_core::Ref::from_raw_ref(&self.0);
        let ref_cipher_2: cpp_core::Ref<CLHSMCipherTextOfCLHSMqk> = cpp_core::Ref::from_raw_ref(&other.0);

        let result =
            CLHSMCipherTextOfCLHSMqk::from_c_l_h_s_mqk_c_l_h_s_m_qk2_c_l_h_s_m_cipher_text_of_c_l_h_s_mqk(ref_c, ref_cipher_1, ref_cipher_2);
        CiphertextBox(result)
    }
}
...
impl Clone for CLBox {
    fn clone(&self) -> Self {
        let ref_self_clhsmqk: cpp_core::Ref<CLHSMqk> = unsafe{cpp_core::Ref::from_raw_ref( &self.0)};
        let new_clhsmqk = unsafe{CLHSMqk::new_copy(ref_self_clhsmqk)};
        CLBox(new_clhsmqk)
    }
}
...
unsafe fn rust_vec_to_mpz(vec: Vec<u8>) -> cpp_core::CppBox<Mpz> {
    let vec_cpp = unsafe{rust_vec_to_cpp(vec)};
    let ref_ucharvec_cpp: cpp_core::Ref<VectorOfUchar> = unsafe {cpp_core::Ref::from_raw_ref(&vec_cpp)};
    let mpz = unsafe{Mpz::from_vector_of_uchar(ref_ucharvec_cpp)};
    return mpz;
}